...
Shader "Custom/RippleWater" {
	Properties {
		//Color picker for saturation of game object.
		_Color ("Color", Color) = (1,1,1,1)
		//Plugin box for texture file.
		_MainTex ("Base (RGB)", 2D) = "white" {}
		//Plugin box for AO file.
		//_AO ("AO", 2D) = "ao" {}
		...
		//Controls wave frequency.  Think in terms of radio waves.... but water.  
		_Frequency ("Frequency", float) = 1
		//Controls wave normal strength.
  ...
  [HideInInspector]_Distance8 ("Distance8", float) = 0
	...
	SubShader {
		...
		//Sets target to 6.0 to work best with lighting.
		#pragma target 6.0

		//Pull 2d texture from MainTex plugin box.
		sampler2D _MainTex;
		//Pull normal map from BumpMap plugin box.
		sampler2D _BumpMap;
		//Pull glossiness value from gloss slider.
		half _Glossiness;
		//Pull metallic value from metal slider.
		half _Metallic;
		//Pulls the values to form wave from Scale, Speed and Frequency.
		...
		half value0 = _Scale * sin(_Time.w * _Speed + offsetvert * _Frequency);
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			o.Albedo = c.rgb;
			//AO comes from the AO texture file.
			//o.AO 
